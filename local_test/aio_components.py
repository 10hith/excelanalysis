...
class MarkdownWithColorAIO(html.Div):  # html.Div will be the "parent" component
    ...
    def __init__(
        ...
        super().__init__([  # Equivalent to `html.Div([...])`
            ...
            dbc.Button(id=self.ids.addRed(aio_id), children="add Red", className="btn-primary"),
        ])
    ...
    @callback(
        ...
        Input(ids.addRed(MATCH), 'n_clicks'),
        State(ids.dropdown(MATCH), 'options'),
        prevent_initial_call=True
    )
    def add_red(n_clicks, drop_down_options):
